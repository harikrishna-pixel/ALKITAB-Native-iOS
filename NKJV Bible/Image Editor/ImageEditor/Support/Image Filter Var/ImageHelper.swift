//
//  ImageHelper.swift
//  sample-instagram
//
//  Created by admin on 2017/12/11.
//  Copyright © 2017 i05nagai. All rights reserved.
//

import Foundation
import UIKit


typealias FilterApplierType = ((_ image: CIImage) -> CIImage?) 

class ImageHelper {
    static func getConstantColorFilter() -> CIFilter? {
        let color = CIFilter(
            name: "CIConstantColorGenerator",
            parameters:[
                kCIInputColorKey: CIColor(red: 1.0, green: 1.0, blue: 1.0, alpha: 0.5),
                ])
        
        return color
    }
    
    static func getColor(
        red: Int,
        green: Int,
        blue: Int,
        alpha: Int = 255) -> CIColor {
        return CIColor(
            red: CGFloat(Double(red) / 255.0),
            green: CGFloat(Double(green) / 255.0),
            blue: CGFloat(Double(blue) / 255.0),
            alpha: CGFloat(Double(alpha) / 255.0))
    }
    
    static func getColorImage(
        red: Int,
        green: Int,
        blue: Int,
        alpha: Int = 255,
        rect: CGRect) -> CIImage {
        let color = self.getColor(
            red: red, green: green, blue: blue, alpha: alpha)
        return CIImage(color: color).cropped(to: rect)
    }
    
    static func applyClarendonFilter(foregroundImage: CIImage) -> CIImage? {
        let backgroundImage = getColorImage(
            red: 127, green: 187, blue: 227, alpha: Int(255 * 0.2), rect: foregroundImage.extent)
        return foregroundImage
            .applyingFilter("CIOverlayBlendMode", parameters: [
                "inputBackgroundImage": backgroundImage,
                ])
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 1.35,
                "inputBrightness": 0.05,
                "inputContrast": 1.1,
                ])
    }
    
    static func applyNashvilleFilter(foregroundImage: CIImage) -> CIImage? {
        let backgroundImage = getColorImage(
            red: 247, green: 176, blue: 153, alpha: Int(255 * 0.56), rect: foregroundImage.extent)
        let backgroundImage2 = getColorImage(
            red: 0, green: 70, blue: 150, alpha: Int(255 * 0.4), rect: foregroundImage.extent)
        return foregroundImage
            .applyingFilter("CIDarkenBlendMode", parameters: [
                "inputBackgroundImage": backgroundImage,
                ])
            .applyingFilter("CISepiaTone", parameters: [
                "inputIntensity": 0.2,
                ])
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 1.2,
                "inputBrightness": 0.05,
                "inputContrast": 1.1,
                ])
            .applyingFilter("CILightenBlendMode", parameters: [
                "inputBackgroundImage": backgroundImage2,
                ])
    }
    
    static func apply1977Filter(ciImage: CIImage) -> CIImage? {
        let filterImage = getColorImage(
            red: 243, green: 106, blue: 188, alpha: Int(255 * 0.1), rect: ciImage.extent)
        let backgroundImage = ciImage
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 1.3,
                "inputBrightness": 0.1,
                "inputContrast": 1.05,
                ])
            .applyingFilter("CIHueAdjust", parameters: [
                "inputAngle": 0.3,
                ])
        return filterImage
            .applyingFilter("CIScreenBlendMode", parameters: [
                "inputBackgroundImage": backgroundImage,
                ])
            .applyingFilter("CIToneCurve", parameters: [
                "inputPoint0": CIVector(x: 0, y: 0),
                "inputPoint1": CIVector(x: 0.25, y: 0.20),
                "inputPoint2": CIVector(x: 0.5, y: 0.5),
                "inputPoint3": CIVector(x: 0.75, y: 0.80),
                "inputPoint4": CIVector(x: 1, y: 1),
                ])
    }
    
    static func applyToasterFilter(ciImage: CIImage) -> CIImage? {
        let width = ciImage.extent.width
        let height = ciImage.extent.height
        let centerWidth = width / 2.0
        let centerHeight = height / 2.0
        let radius0 = min(width / 4.0, height / 4.0)
        let radius1 = min(width / 1.5, height / 1.5)
        print(width, height, centerWidth, centerHeight, radius0, radius1)
        
        let color0 = self.getColor(red: 128, green: 78, blue: 15, alpha: 255)
        let color1 = self.getColor(red: 79, green: 0, blue: 79, alpha: 255)
        let circle = CIFilter(name: "CIRadialGradient", parameters: [
            "inputCenter": CIVector(x: centerWidth, y: centerHeight),
            "inputRadius0": radius0,
            "inputRadius1": radius1,
            "inputColor0": color0,
            "inputColor1": color1,
            ])?.outputImage?.cropped(to: ciImage.extent)
        
        return ciImage
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 1.0,
                "inputBrightness": 0.01,
                "inputContrast": 1.1,
                ])
            .applyingFilter("CIScreenBlendMode", parameters: [
                "inputBackgroundImage": circle!,
                ])
    }
 
    
    static func applyHazeRemovalFilter(image: CIImage) -> CIImage? {
        let filter = HazeRemovalFilter()
        filter.inputImage = image
        return filter.outputImage
    }
    
    static func createDefaultFilterApplier(name: String) -> FilterApplierType {
        return { (image: CIImage) -> CIImage? in
            let filter = CIFilter(name: name)
            filter?.setValue(image, forKey: kCIInputImageKey)
            return filter?.outputImage!
        }
    }
    
    
    
    
    
    
    
    static func OriGinalImage(foregroundImage: CIImage) -> CIImage? {
        return foregroundImage
    }
    // 15 3
    
    
    
    
    
    
    static func FalseColorImage(ciImage: CIImage) -> CIImage? {
        return ciImage
            .applyingFilter("CIFalseColor", parameters: [
                "inputColor0": CIColor.yellow,
                "inputColor1": CIColor.blue
                ])
    }

    
    static func GammaAdjus(ciImage: CIImage) -> CIImage? {
        
        return ciImage
            .applyingFilter("CIGammaAdjust", parameters: [
                "inputPower": 1.0
                ])
    }
    
    
    static func ColorInvert(ciImage: CIImage) -> CIImage? {
        
        return ciImage
            .applyingFilter("CIColorInvert")
    }
    
    
    
    static func SepiaTone(ciImage: CIImage) -> CIImage? {
        
        return ciImage
            .applyingFilter("CISepiaTone", parameters: [
                "inputIntensity": 1.0
                ])
    }
    
//    static func GlassDistortion(ciImage: CIImage) -> CIImage? {
//
//        return ciImage
//            .applyingFilter("CIGlassDistortion", parameters: [
//                "inputIntensity": 1.0
//                ])
//    }
//
    
    
    static func Bloom(ciImage: CIImage) -> CIImage? {
        
        return ciImage
            .applyingFilter("CIBloom", parameters: [
                "inputRadius": 10.0,
                "inputIntensity": 1.0
                ])
    }
    
    
    // New Filters
    
    
    static func BlackFilter1(ciImage: CIImage) -> CIImage? {
        
        return ciImage
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 0.0,
                "inputContrast": 1.37,
                ])
    }
    
    

    
    
    static func BlackFilter2(ciImage: CIImage) -> CIImage? {
        
        return ciImage
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 0.0,
                ])
            .applyingFilter("CISharpenLuminance", parameters: [
                "inputSharpness": 0.94,
                "inputRadius": 1.69,
                ])
    }
    
    static func BlackFilter3(ciImage: CIImage) -> CIImage? {
        return ciImage
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 0.0,
                ])
            .applyingFilter("CISepiaTone", parameters: [
                "inputIntensity": 0.2,
                ])
    }
    
    static func BlackFilter4(ciImage: CIImage) -> CIImage? {
        return ciImage
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 0.0,
                ])
            .applyingFilter("CIBloom", parameters: [
                "inputRadius": 3.0,
                "inputIntensity": 0.6
                ])
    }
    
    
    static func BlackFilter5(ciImage: CIImage) -> CIImage? {
        let backgroundImage = ciImage
        return ciImage
            .applyingFilter("CIPhotoEffectChrome", parameters: [
                "inputImage": backgroundImage,
                ])
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 0.0,
                ])
    }
    
    
     
    
    
    
    static func FadeFilter1(ciImage: CIImage) -> CIImage? {
        
        return ciImage
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 0.36,
                "inputContrast": 1.3,
                ])
        
            .applyingFilter("CISharpenLuminance", parameters: [
                "inputSharpness": 0.94,
                "inputRadius": 1.69,
                ])
    }
    
    
    
    
    
    
    static func zoomBlurImage(ciImage: CIImage) -> CIImage? {

        let filter = CIFilter(name: "CIZoomBlur")!
        
        filter.setDefaults()
        filter.setValue(ciImage, forKey: kCIInputImageKey)
        filter.setValue(CIVector(x: ciImage.extent.size.width/2, y: ciImage.extent.size.height/2), forKey: kCIInputCenterKey)
        filter.setValue(10, forKey: "inputAmount")
        
        if let outputImage = filter.value(forKey: kCIOutputImageKey) as? CIImage {
            return outputImage
        }
        return nil
    }
    
    
    
    static func zoomBlurImage1(ciImage: CIImage) -> UIImage? {

        let filter = CIFilter(name: "CIZoomBlur")!
        
        filter.setDefaults()
        filter.setValue(ciImage, forKey: kCIInputImageKey)
        filter.setValue(CIVector(x: ciImage.extent.size.width/2, y: ciImage.extent.size.height/2), forKey: kCIInputCenterKey)
        filter.setValue(10, forKey: "inputAmount")
        
        if let outputImage = filter.value(forKey: kCIOutputImageKey) as? UIImage {
            return outputImage
        }
        return nil
    }
    
    
    
    
    
    
    
    
    
    
    
    static func freeSizeFilter(ciImage: CIImage) -> CIImage? {
        let width = ciImage.extent.width
        let height = ciImage.extent.height
        let centerWidth = width / 2.0
        let centerHeight = height / 2.0
        let radius0 = min(width / 4.0, height / 4.0)
        let radius1 = min(width / 1.5, height / 1.5)
        print(width, height, centerWidth, centerHeight, radius0, radius1)
        
        let color0 = self.getColor(red: 128, green: 78, blue: 15, alpha: 255)
        let color1 = self.getColor(red: 79, green: 0, blue: 79, alpha: 255)
        let circle = CIFilter(name: "CIRadialGradient", parameters: [
            "inputCenter": CIVector(x: centerWidth, y: centerHeight),
            "inputRadius0": radius0,
            "inputRadius1": radius1,
            "inputColor0": color0,
            "inputColor1": color1,
            ])?.outputImage?.cropped(to: ciImage.extent)
        
        return ciImage
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 1.0,
                "inputBrightness": 0.01,
                "inputContrast": 1.1,
                ])
    }
    
    static func freeSizeFilter1(ciImage: CIImage) -> CIImage? {
        let filterImage = getColorImage(
            red: 240, green: 204, blue: 184, alpha: 1, rect: ciImage.extent)
        let backgroundImage = ciImage
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 1.3,
                "inputBrightness": 0.1,
                "inputContrast": 1.35,
                ])
            .applyingFilter("CIHueAdjust", parameters: [
                "inputAngle": 0.3,
                ])
            .applyingFilter("CISharpenLuminance", parameters: [
                "inputSharpness": 0.94,
                "inputRadius": 1.69,
                ])
        return filterImage
            .applyingFilter("CIScreenBlendMode", parameters: [
                "inputBackgroundImage": backgroundImage,
                ])
            .applyingFilter("CIToneCurve", parameters: [
                "inputPoint0": CIVector(x: 0, y: 0),
                "inputPoint1": CIVector(x: 0.25, y: 0.20),
                "inputPoint2": CIVector(x: 0.5, y: 0.5),
                "inputPoint3": CIVector(x: 0.75, y: 0.80),
                "inputPoint4": CIVector(x: 1, y: 1),
                ])
    }
    
    
    static func freeSizeFilter2(ciImage: CIImage) -> CIImage? {

        let filter = CIFilter(name: "CIBokehBlur")!
        return ciImage
            .applyingFilter("CIBokehBlur", parameters: [
                "inputRingAmount": 1.5,
                "inputRingSize": 1.4,
                "inputSoftness": 2.0,
                "inputRadius": 1.0,
                ])
        if let outputImage = filter.value(forKey: kCIOutputImageKey) as? CIImage {
            return outputImage
        }
        return nil
    }
    
    
        
    static func Filter2(ciImage: CIImage) -> CIImage? {
        let filterImage = getColorImage(
            red: 255, green: 246, blue: 238, alpha: Int(255 * 0.1), rect: ciImage.extent)
        let backgroundImage = ciImage
            .applyingFilter("CIHueAdjust", parameters: [
                "inputAngle": 0.0,
                ])
            .applyingFilter("CISharpenLuminance", parameters: [
                "inputSharpness": 0.5,
                "inputRadius": 1.0,
                ])
            .applyingFilter("CIColorControls", parameters: [
                "inputBrightness": -0.05,
                "inputContrast": 1.1,
                ])
        return filterImage
            .applyingFilter("CIScreenBlendMode", parameters: [
                "inputBackgroundImage": backgroundImage,
                ])
            .applyingFilter("CIToneCurve", parameters: [
                "inputPoint0": CIVector(x: 0, y: 0),
                "inputPoint1": CIVector(x: 0.25, y: 0.20),
                "inputPoint2": CIVector(x: 0.5, y: 0.5),
                "inputPoint3": CIVector(x: 0.75, y: 0.80),
                "inputPoint4": CIVector(x: 1, y: 1),
                ])

    }
    
    
    static func Filter3(ciImage: CIImage) -> CIImage? {
        let filterImage = getColorImage(
            red: 255, green: 249, blue: 204, alpha: Int(255 * 0.1), rect: ciImage.extent)
        let backgroundImage = ciImage
            .applyingFilter("CIHueAdjust", parameters: [
                "inputAngle": 0.0,
                ])
            .applyingFilter("CISharpenLuminance", parameters: [
                "inputSharpness": 0.8,
                "inputRadius": 1.0,
                ])
            .applyingFilter("CIColorControls", parameters: [
                "inputBrightness": -0.06,
                "inputContrast": 1.1,
                ])
        return filterImage
            .applyingFilter("CIScreenBlendMode", parameters: [
                "inputBackgroundImage": backgroundImage,
                ])
            .applyingFilter("CIToneCurve", parameters: [
                "inputPoint0": CIVector(x: 0, y: 0),
                "inputPoint1": CIVector(x: 0.25, y: 0.20),
                "inputPoint2": CIVector(x: 0.5, y: 0.5),
                "inputPoint3": CIVector(x: 0.75, y: 0.80),
                "inputPoint4": CIVector(x: 1, y: 1),
                ])
    }
    
    
    static func Filter4(ciImage: CIImage) -> CIImage? {
        let filterImage = getColorImage(
            red: 133, green: 157, blue: 182, alpha: Int(255 * 0.1), rect: ciImage.extent)
        let backgroundImage = ciImage
            .applyingFilter("CIHueAdjust", parameters: [
                "inputAngle": 0.0,
                ])
            .applyingFilter("CISharpenLuminance", parameters: [
                "inputSharpness": 0.8,
                "inputRadius": 1.0,
                ])
            .applyingFilter("CIColorControls", parameters: [
                "inputBrightness": 0.02,
                ])
        return filterImage
            .applyingFilter("CIScreenBlendMode", parameters: [
                "inputBackgroundImage": backgroundImage,
                ])
            .applyingFilter("CIToneCurve", parameters: [
                "inputPoint0": CIVector(x: 0, y: 0),
                "inputPoint1": CIVector(x: 0.25, y: 0.20),
                "inputPoint2": CIVector(x: 0.5, y: 0.5),
                "inputPoint3": CIVector(x: 0.75, y: 0.80),
                "inputPoint4": CIVector(x: 1, y: 1),
                ])
    }
    
    
     

// New values
    
    static func Filter1(ciImage: CIImage) -> CIImage? {
        let filterImage = getColorImage(
            red: 255, green: 221, blue: 198, alpha: Int(255 * 0.1), rect: ciImage.extent)
        let backgroundImage = ciImage
            .applyingFilter("CIColorControls", parameters: [
                "inputSaturation": 0.36,
                "inputContrast": 1.05,
                ])
        return filterImage
            .applyingFilter("CIScreenBlendMode", parameters: [
                "inputBackgroundImage": backgroundImage,
                ])
    }
//
    
    
    
}
